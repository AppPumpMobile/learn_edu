...
class HttpService {
  ...
  Future<bool> login(
    ...
    AndroidDeviceInfo androidDeviceInfo = await deviceInfoPlugin.deviceInfo;
    http.Response res = await http.post(Uri.parse(APIData.login), body: {
      "email": email,
      "password": pass,
      'mac_user': '${androidDeviceInfo.id}'
    });
    if (res.statusCode == 200) {
  ...
  Future<bool> signUp(String name, String email, String password, String mobile,
    ...
    if(Platform.isAndroid){
      ...
      deviceId = windowsDeviceInfo.computerName ;
    }
    ...
    // print(windowsDeviceInfo.computerName);
