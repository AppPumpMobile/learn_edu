...
class _LessonsState extends State<Lessons> {
      ...
      child: InkWell(
        ...
        onTap: () {
          if (_allClips != null && _allClips.length > 0 && haveVideos) {
            print("this is before going to PlayListScreen where playIcon");
  ...
  Widget doneicon(List<String> marksSecs, List<Section> sections, int chpId) {
    ...
    return InkWell(
      onTap: () {
        if (_allClips != null && _allClips.length > 0 && haveVideos) {
          print("this is before going to PlayListScreen where doneicon");
  ...
  List<Widget> _buildexpandablewidget(
    ...
    playVideo(idx) {
      ...
      print("this is before going to PlayListScreen where _buildexpandablewidget");
      for(int i = 0 ; i < sections.length ; i++){
        print("this is sections data: ${sections[i].sectionDetails.chapterName}");
      }
      for(int i = 0 ; i < _allClips.length ; i++){
        print("this is _allClips data: ${_allClips[i].title}");
        print("this is _allClips data: ${_allClips[i].parent}");
      }

      for(int i = 0 ; i < widget.details.course.chapter.length ; i++){
        print("this is widget.details.course.chapter data: ${widget.details.course.chapter[i].chapterName}");
      }

      print("this is widget.details.course.title data: ${widget.details.course.url}");
        print("this is widget.progress data: ${widget.progress}");
